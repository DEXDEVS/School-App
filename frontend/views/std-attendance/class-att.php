                        ...
                        for( $std=0; $std<$studentLength; $std++) {  ?>
                            <tr>
                                ...
                                <?php $stdId = $student[$std]['std_enroll_detail_std_id'];
                                     ...
                                     // $subjectName = Yii::$app->db->createCommand("SELECT subject_alias FROM subjects  WHERE subject_id = '$stdId'")->queryAll(); 
                                ...
                                for ($r=0; $r <$subjectCount ; $r++) {
                                    ...
                                    for ($s=0; $s <$subjectlength ; $s++) { 
                                        ?> 
                                    <td><?php
                                        $c = count($attendanceArr[$r]);
                                            //if ($r < $c){
                                                if($subjectAlias[$s] == 'D'){
                                                        if(empty($c)){
                                                            echo "-";
                                                        } else {
                                                            if(empty($attendanceArr[$r][$std]['status'])){
                                                                echo "-";
                                                            } else {
                                                            $status = $attendanceArr[$r][$std]['status'];
                                                            //echo $status;  
                                                            //echo $r.$std;
                                                            }  
                                                        }
...
$subId = $attendanceArr[$r][$std]['subject_id'];
                                                $subject_alias = Yii::$app->db->createCommand("SELECT subject_alias FROM subjects  WHERE subject_id = '$subId'")->queryAll();
                                                if($subjectAlias[$s] == $subject_alias[0]['subject_alias'] &&  $attendanceArr[$r][$std]['student_id'] == $stdId){
                                                    echo $subjectAlias[$s];
