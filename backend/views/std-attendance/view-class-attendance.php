...
use backend\controllers\SmsController;
    ...
    if (isset($_POST["save"])) {
        ...
        //$empId = Yii::$app->db->createCommand("SELECT emp.teacherHeadId FROM emp_info as emp WHERE emp.emp_cnic = '$empCnic' AND emp.emp_branch_id = '$branch_id'")->queryAll();
        //$teacher_id = $empId[0]['teacherHeadId'];
...
<div class="container-fluid">
    <div class="row">
        <div class="col-md-12">
            <!-- back button start -->
             <ol class="breadcrumb">
              <li><a class="btn btn-primary btn-xs" href="take-attendance?sub_id=<?php echo $sub_id;?>&class_id=<?php echo $classid;?>&teacherHeadId=<?php echo $teacherHeadId;?>"><i class="fa fa-backward"></i> Back</a></li>
            </ol>
            <!-- back button close -->
        </div>
    </div>
                        ...
                        for( $std=0; $std<$studentLength; $std++) {  ?>
                            <tr>
                                ...
                                for ($r=0; $r <$subjectCount ; $r++) {
                                    ...
                                    for ($s=0; $s <$subjectlength ; $s++) {
                                        $subId = $subjectId[$s]; 
                                        $attend = Yii::$app->db->createCommand("SELECT CAST(date AS DATE),subject_id,status,student_id FROM std_attendance WHERE class_name_id = '$classid' AND session_id = '$sessionid' AND section_id = '$sectionid' AND CAST(date AS DATE) = '$date' AND subject_id = '$subId' AND branch_id = '$branch_id' AND teacher_id = '$teacherHeadId'")->queryAll();
                                        $attendanceArr[$s] = $attend; 
                                        //print_r($attend);
                                        ?> 
                                    <td><?php
...
if(isset($_POST['update'])){
    ...
    $stdAttendId = $_POST["stdAttendance"];
                ...
                try{
                    ...
                    $attendance = Yii::$app->db->createCommand()->update('std_attendance',                [
                            'status'            => $status[$i],
                            'updated_at'        => new \yii\db\Expression('NOW()'),
                            'updated_by'        => Yii::$app->user->identity->id,
                            ],
                            ['branch_id' => $branch_id, 'teacher_id' => $teacherHeadId, 'class_name_id' => $classid, 'session_id' => $sessionid,'section_id' => $sectionid, 'subject_id'=> $sub_id, 'CAST(date AS Date)' => $date, 'student_id'=> $stdAttendId[$i],]
                        )->execute();
                    }
                    ...
                    $transection->rollback();
                    echo $e;
                    Yii::$app->session->setFlash('warning', "Attendance not upated. Try again!");
                }
...
<div class="container-fluid">
    <div class="row">
        <div class="col-md-12">
            <!-- back button start -->
             <ol class="breadcrumb">
              <li><a class="btn btn-primary btn-xs" href="update-attendance?sub_id=<?php echo $sub_id;?>&class_id=<?php echo $classid;?>&teacherHeadId=<?php echo $teacherHeadId;?>"><i class="fa fa-backward"></i> Back</a></li>
            </ol>
            <!-- back button close -->
        </div>
    </div>
